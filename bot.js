/****************************************
 *   RESBot is made using the AstralMod Source Code
 *
...
function setGame() {
    ...
    switch (Math.floor(Math.random() * 1000) % 22) {
        case 0:
            presence.game.name = "with the ball";
            break; //SCRUATCHO
        case 1:
            presence.game.name = "Ro Evolution Soccer";
            break;
        case 2:
            presence.game.name = "Annoy PhantomRayV";
            break;
        case 3:
        ...
        case 13:
            ...
            break;
        case 14:
            presence.game.name = "drawing ghosts";
            break;
        case 15:
            presence.game.name = "obsessing with hugz";
            break;
        case 16:
        ...
        case 20:
            ...
            break;
        case 21:
            presence.game.name = "mod:help for more info";
            break;
    }
...
client.on('ready', () => {
    console.log("RESBot is now ready!");
...
function messageChecker(oldMessage, newMessage) {
    ...
    if (panicMode[message.guild.id]) {
        if (msg == "mod:panic" && (message.member.roles.find("name", "Admin")  || message.member.roles.find("name", "Upper Council of Explorers") || message.member.roles.find("name","Owner"))) {
    ...
    if (message.author.id != 280495817901473793 && !message.author.bot) {
        ...
        if (message.mentions != null && message.mentions.users != null) {
            if (message.mentions.users.has("301663734764535808")) {
            ...
            switch (command) {
                ...
                case "help":
                    message.channel.send(
                        ...
                        "about             Tells you about RESBot\n" + 
                        "copyright         Tells you about RESBot\n" + 
                        "license           Tells you about RESBot\n" + 
                        "warranty          Tells you about RESBot\n\n" + 
                        "ping|pong         Asks RESBot to reply with a message\n\n" +
                ...
                case "copyright":
                    message.author.sendMessage(
						"RESBot is part of the AstralMod Source Code\n\n" +
                        "Copyright (C) 2017 Victor Tran and Rylan Arbour\n\n" +
                ...
                case "honeyfries":
                    if (message.guild.id == 277922530973581312) {
                        ...
                        message.channel.send(':no_entry_sign: Honeyfries have nothing to do with RES. Carry on...');
                    }
                    message.delete();
                    commandProcessed = true;
                    break;
        ...
        if (msg.toLowerCase().startsWith("mod:") && !commandProcessed) {
            ...
            if (message.member.roles.find("name", "Admin") || message.member.roles.find("name", "Moderator") || message.member.roles.find("name", "moderators") || message.member.roles.find("name", "Mod") || message.member.roles.find("name", "Upper Council of Explorers") || message.member.roles.find("name", "Lower Council of Explorers") || message.member.roles.find("name","Owner")) { //Thanks Aren! :D
                var command = msg.substr(4);
                switch (command) {
                    case "filter":
                        if (message.guild.id != 277922530973581312) {
                            message.reply(':no_entry_sign: ERROR: RES does not need a filter. ask Jed if you want this enabled .');
                        } else {
                            if (expletiveFilter) {
                    ...
                    case "mod off":
                        ...
                        break;
                    case "panic":
                        if (message.member.roles.find("name", "Admin") || message.member.roles.find("name", "Upper Council of Explorers") || message.member.roles.find("name","Owner")) {
                            ...
                            message.reply(':no_entry_sign: NO: This is an admin/owner only command.');
                            message.delete();
                        }
                        break;
                    case "interrogate":
                        if (message.guild.id != 277922530973581312 && message.guild.id != 234414439330349056) {
                            message.reply(':no_entry_sign: ERROR: RES does not have a interrogation channel, create one and then ask Jed to enable it..');
                        } else {
                    ...
                    case "reboot":
                        message.channel.send(":white_check_mark: We'll be back in a bit.").then(function() {
                            ...
                            client.destroy();
                            client.login('MzAxNjYzNzM0NzY0NTM1ODA4.C8-RwQ.enLCbxDixVrDpbSlFXfnjKTWypc').then(function() {
								console.log("Rebooted");
                                message.channel.send(":white_check_mark: RESBot is back online!");
                            }).catch(function() {
                    ...
                    case "jail":
                        if (message.guild.id != 277922530973581312) {
                            message.reply(':no_entry_sign: ERROR: RES does not have a jail, create one then ask Jed to enable this function.');
                        } else {
                            if (jailMember == null) {
                    ...
                    case "help":
                        ...
                        helpMessage = helpMessage + 
                            ...
                            "reboot            Asks RESBot to reconnect.\n" +
                            "\n" +
                    ...
                    case "banterrogate":
                        if (message.guild.id != 277922530973581312 && message.guild.id != 234414439330349056) {
                            message.reply(':no_entry_sign: ERROR: RESBot can not banterrogate on this server.');
                        } else {
                            if (interrogMember == null) {
                                message.reply(':no_entry_sign: ERROR: No user to banterrogate. See mod:help for more information.');
                            } else {
                                if (interrogMember.guild.id == 277922530973581312) {
                                    interrogMember.sendMessage("You seem to be someone that has been making alts. If you're not, then to appeal, get in touch with MrJed385#3013. Sorry about the kick. We've had to do this because of a special someone trying to break the rules.");
                                    interrogMember.ban();
                ...
                if (command == "poweroff") {
                    if (message.author.id == 114465624821989376) {
                        if (poweroff) {
                            switch (Math.floor(Math.random() * 1000) % 3) {
                                case 0:
                                    message.channel.send(':white_check_mark: RESBot is now exiting. Goodbye!').then(function() {
                                        process.exit(0);
                                    }).catch(function() {
                                        process.exit(0);
                                    });
                            ...
                            message.channel.send(':information_source: If you\'re just trying to stop RESBot from moderating, use `mod:mod off` instead. Otherwise, to power off RESBot, type in `mod:poweroff` again.');
                            poweroff = true;
                        }
                    } else {
                        message.reply(':no_entry_sign: NO: Only <@114465624821989376> is allowed to power off the bot. To turn off moderation, use `mod:mod off`.');
                    }
                } else {
                    poweroff = false;
                }
                ...
                message.reply(':no_entry_sign: NO: What? You\'re not a member of the RES staff! Why would you be allowed to type that!?');
                message.delete();
            }
        }
